...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch latest role_dna_fit_scores for this user + job
    const { data: fitScore, error: fetchError } = await supabaseClient
    ...
    if (fetchError || !fitScore) {
      // No fit score exists yet
      return new Response(
    ...
    return new Response(
      JSON.stringify({
        ...
        fit: {
          ...
          fit_dimension_scores: fitScore.fit_dimension_scores,
          summary: fitScore.summary,
          createdAt: fitScore.created_at,
        }
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
    ...
    console.error("Error in get-role-dna-fit:", error);
